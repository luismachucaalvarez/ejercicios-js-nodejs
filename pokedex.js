...
app.factory("Pokedex", function(rp) {
...
// Función que me permita buscar por nombre en el array de pokemon 
// y me devuelva el id -- Lo malo es que venga vacio no exista devuelve null

    Pokedex.prototype.findIdByName = function(name) {
        if (name == null) return null;

        var pokemon = this.pokemons.find(function(pokemon) {
...
// Función que permite buscar por el ID del pokemon, los datos en la appi
// que se encuentra en un repo externo.
    Pokedex.prototype.buscarPokemon = function (pokemonName) {
        var pokemonId = this.findIdByName(pokemonName); // con esto busco en la api
        poke = rp(this.api + "/api/v2/pokemon/" + pokemonId)
        .then(console.log(poke.name + " "  + " el ID  es " + pokemonId + "y el peso es " + poke.weight))
        .catch(console.log ("no logré encontrarlo"));
    }
...
/*
    Pokedex.prototype.buscarPokemon = function(pokemonName) {
        ...
        var promisePokemon = $http.get(this.apiUrl + "/api/v1/pokemon/" + pokemonId);
        promisePokemon
            .success(this.getPokemonPhoto.bind(this))
        return promisePokemon;
    }

    Pokedex.prototype.getPokemonPhoto = function(pokemon) {
        var self = this;
        $http
            .get(this.apiUrl + pokemon.sprites[0].resource_uri)
            .success(function(photoInfo) {
                pokemon.photo = self.apiUrl + photoInfo.image;
            })
    }
